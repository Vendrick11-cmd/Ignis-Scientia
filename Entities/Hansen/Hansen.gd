...
func _fight(delta):
	if Input.is_action_just_pressed("walk") and PlayerGlobal.walk == false:
		PlayerGlobal.walk = true
	elif Input.is_action_just_pressed("walk") and PlayerGlobal.walk == true:
		PlayerGlobal.walk = false
	...
	if Input.is_action_pressed("RIGHT") and PlayerGlobal.walk == false:
		motion.x = JOGSPEED
		state_machine.travel("THJog")
	elif Input.is_action_pressed("RIGHT") and PlayerGlobal.walk == true:
		motion.x = SIMPLESPEED
		state_machine.travel("THStep")
		
		
	elif Input.is_action_pressed("LEFT") and PlayerGlobal.walk == false:
		motion.x = -JOGSPEED
		state_machine.travel("THJog")
	elif Input.is_action_pressed("LEFT") and PlayerGlobal.walk == true:
		motion.x = -SIMPLESPEED
		state_machine.travel("THStep")
	else : 
		motion.x = 0
