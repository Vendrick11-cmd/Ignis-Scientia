...
func _process(_delta):
	...
	if PlayerGlobal.shield_buckler == false :
		$HemwickSword/HandShield.hide()
		$HemwickSword/ShieldBuckler.hide()
	elif PlayerGlobal.shield_buckler == true :
	...
	if PlayerGlobal.shield_heater == false :
		$HemwickSword/HandShield.hide()
		$HemwickSword/ShieldHeater.hide()
	elif PlayerGlobal.shield_heater == true :
		$HemwickSword/HandShield.show()
		$HemwickSword/ShieldHeater.show()
		$HemwickSword/ShieldBuckler.hide()

	if PlayerGlobal.shield_buckler_hp <= 0 :
		PlayerGlobal.shield_buckler = false
		$HemwickSword/HandShield.hide()
		$HemwickSword/ShieldBuckler.hide()
		if PlayerGlobal.shield_buckler == false :
			PlayerGlobal.shield_buckler_hp = 25
	if PlayerGlobal.shield_heater_hp <= 0 :
		PlayerGlobal.shield_heater = false
		$HemwickSword/HandShield.hide()
		$HemwickSword/ShieldhHeater.hide()
		if PlayerGlobal.shield_heater == false :
			PlayerGlobal.shield_heater_hp = 50
...
func _fight(_delta) :
	...
	if PlayerGlobal.in_dialogue == false:
		...
		if !is_on_floor():
			...
			if motion.y < 0 or motion.y == 0 or motion.y > 0 and !is_on_floor() and !$HangL.is_colliding() and !$HangR.is_colliding():
				...
				if Input.is_action_pressed("DOwN") and Input.is_action_just_pressed("HeavyAttack") and PlayerGlobal.Slam == true:
					state = AIRATTACKSLAM
		...
		if is_on_floor() :
			...
			if Input.is_action_pressed("Block") :
				if PlayerGlobal.shield_buckler == true or PlayerGlobal.shield_heater == true :
...
func _combo2(_delta):
	...
	if $ComboTimerDgr.time_left > 0.05 :
		...
		if Input.is_action_just_pressed("HeavyAttack") and PlayerGlobal.HeavyChain == true:
			if PlayerGlobal.Current_Equipped == "dagger":
				stats.stamina -= PlayerGlobal.dagger_stamina *2
			elif PlayerGlobal.Current_Equipped == "unarmed":
				stats.stamina -= PlayerGlobal.unarmed_stamina*2
	...
	if $HemwickSword/Shield/RayShield.is_colliding() or $HemwickSword/Shield/RayShield2.is_colliding() or $HemwickSword/Shield/RayShield3.is_colliding() :
		...
		if PlayerGlobal.shield_buckler == true :
			if state == BLOCK :
				...
				PlayerGlobal.shield_buckler_hp -= area.damage / 2
				PlayerGlobal.camera.shake(25)
				if stats.stamina <= 0 :
		...
		if PlayerGlobal.shield_heater == true :
			if state == BLOCK :
				...
				PlayerGlobal.shield_heater_hp -= area.damage / 2
				PlayerGlobal.camera.shake(25)
				if stats.stamina <= 0 :
					stats.health -= area.damage
					$AnimationPlayer.play("Hurt")
...
func _attackheavy(_delta) :
	...
	if PlayerGlobal.Current_Equipped == "dagger":
		...
		$HemwickSword/HitBoxSpear.damage = PlayerStats.base_damage * PlayerGlobal.shortsword_damage + 5
		var anim = randi() % 2
		match anim:
			0:
				state_machine.travel("Shortsword")
...
func _on_HitBox_area_entered(_area):
	...
	frameFreeze(0.1, 0.2)
	PlayerGlobal.camera.shake(400,0.2,150)
