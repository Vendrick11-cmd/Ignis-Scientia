...
#IN VEHICLE
	if $MineCart/ChangeSpeed.is_colliding():
		PlayerGlobal.in_vehicle = true
		$REnd.disabled = false
		$LEnd.disabled = false
	else :
		PlayerGlobal.in_vehicle = false
		$REnd.disabled = true
		$LEnd.disabled = true
...
# NOT IN VEHICLE
	if $MineCart/LEFT.is_colliding() and PlayerGlobal.is_moving == true:
		motion.x += 1.2
		if motion.x > SPEED:
			motion.x = SPEED

	elif $MineCart/RIGHT.is_colliding() and PlayerGlobal.is_moving == true:
		motion.x -= 1.2
		if motion.x < -SPEED:
			motion.x = -SPEED
	else :
