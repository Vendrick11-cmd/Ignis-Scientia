...
func _physics_process(_delta):
	if ready == true and Input.is_action_just_pressed("Interact") and $InteractRay.is_colliding():
		...
		if comfrey == true:
			...
			PlayerGlobal.Name = "2x Comfrey"
			IAdd.start()
			PlayerGlobal.Name = "2x Comfrey Seed"
			IAdd.start()

		elif eyebright == true:
			InventoryLoad.eyebright += 2
			InventoryLoad.eyebright_seed += 2
			PlayerGlobal.Name = "2x Eyebright"
			IAdd.start()
			PlayerGlobal.Name = "2x Eyebright Seed"
			IAdd.start()
		elif garlic == true:
			InventoryLoad.garlic += 2
			InventoryLoad.garlic_seed += 2
			PlayerGlobal.Name = "2x Garlic"
			IAdd.start()
			PlayerGlobal.Name = "2x Garlic Seed"
			IAdd.start()
		elif ginger == true:
			InventoryLoad.ginger += 1
			InventoryLoad.ginger_seed += 2
			PlayerGlobal.Name = "1x Ginger"
			IAdd.start()
			PlayerGlobal.Name = "2x Ginger Seed"
			IAdd.start()
		elif ginseng == true:
			InventoryLoad.ginseng += 1
			InventoryLoad.ginseng_seed += 2
			PlayerGlobal.Name = "2x Ginseng"
			IAdd.start()
			PlayerGlobal.Name = "2x Ginseng Seed"
			IAdd.start()
		elif lavender == true:
			InventoryLoad.lavender += 3
			InventoryLoad.lavender_seed += 2
			PlayerGlobal.Name = "3x Lavender"
			IAdd.start()
			PlayerGlobal.Name = "2x Lavender Seed"
			IAdd.start()
		elif mandrake == true:
			InventoryLoad.mandrake += 1
			InventoryLoad.mandrake_seed += 2
			PlayerGlobal.Name = "1x Mandrake"
			IAdd.start()
			PlayerGlobal.Name = "2x Mandrake Seed"
			IAdd.start()
		elif mugwort == true:
			InventoryLoad.mugwort += 2
			InventoryLoad.mugwort_seed += 2
			PlayerGlobal.Name = "2x Mugwort"
			IAdd.start()
			PlayerGlobal.Name = "2x Mugwort Seed"
			IAdd.start()
		elif nettle == true:
			InventoryLoad.nettle += 3
			InventoryLoad.nettle_seed += 3
			PlayerGlobal.Name = "3x Nettle"
			IAdd.start()
			PlayerGlobal.Name = "3x Nettle Seed"
			IAdd.start()
		elif plantain == true:
			InventoryLoad.plantain += 2
			InventoryLoad.plantain_seed += 3
			PlayerGlobal.Name = "2x Plantain"
			IAdd.start()
			PlayerGlobal.Name = "3x Plantain Seed"
			IAdd.start()
		elif rosemary == true:
			InventoryLoad.rosemary += 2
			InventoryLoad.rosemary_seed_seed += 2
			PlayerGlobal.Name = "2x Rosemary"
			IAdd.start()
			PlayerGlobal.Name = "2x Rosemary Seed"
			IAdd.start()
		elif thistle == true:
			InventoryLoad.thistle += 3
			InventoryLoad.thistle_seed += 2
			PlayerGlobal.Name = "3x Thistle"
			IAdd.start()
			PlayerGlobal.Name = "2x Thistle Seed"
			IAdd.start()
		elif valerian == true:
			InventoryLoad.valerian += 1
			InventoryLoad.valerian_seed += 2
			PlayerGlobal.Name = "1x Valerian"
			IAdd.start()
			PlayerGlobal.Name = "2x Valerian Seed"
			IAdd.start()
		elif yarrow == true:
			InventoryLoad.yarrow += 2
			InventoryLoad.yarrow_seed += 3
			PlayerGlobal.Name = "2x Yarrow"
			IAdd.start()
			PlayerGlobal.Name = "3x Yarrow Seed"
			IAdd.start()
		_reset()
	...
	if Input.is_action_just_pressed("Interact") and $InteractRay.is_colliding() and ready == false and $Planting.current_animation == "None" and $PickUpArea/CollisionShape2D.disabled == false and PlayerGlobal.inventory == false and PlayerGlobal.charactermenu == false and PlayerGlobal.alchemy == false:
