...
func _physics_process(delta):
	if PlayerGlobal.JumpBoost == true:
...
func _on_Button_pressed():
	if PlayerGlobal.skill_selector == 0 and PlayerGlobal.JumpBoost == false :
		if $"../5Combo/Button/AnimationPlayer".current_animation == "Active":
			$Button/AnimationPlayer.play("Click")
			$Descr.visible = false
			PlayerGlobal.skill_selector = 1
			if PlayerGlobal.JumpBoost == false:
				$Sure.visible = true
			else : pass
		else : pass
func _on_Yes_pressed():
	PlayerGlobal.skill_selector = 0
	PlayerGlobal.JumpBoost = true
	$Sure.visible = false
func _on_No_pressed():
	PlayerGlobal.skill_selector = 0
	$Button/AnimationPlayer.play("Idle")
