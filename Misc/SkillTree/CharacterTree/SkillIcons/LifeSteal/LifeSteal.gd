...
func _on_SkillButton_mouse_entered():
	...
	if PlayerGlobal.LifeSteal == true and PlayerGlobal.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("ActiveHover")

func _on_SkillButton_mouse_exited():
	if !$SkillButton/Skill/AnimationPlayer.current_animation == "Click" and PlayerGlobal.LifeSteal == false and $Hover/YesNo/Activate.visible == false and PlayerGlobal.active_skill == false:
...
func _on_SkillButton_pressed():
	if PlayerGlobal.HealthPick == true:
		...
		if PlayerGlobal.skill_selector == 0 and PlayerGlobal.LifeSteal == false:
