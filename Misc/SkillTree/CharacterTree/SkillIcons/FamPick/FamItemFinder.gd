...
func _physics_process(delta):
	if PlayerGlobal.FamItemFinder == true:
...
func _on_Button_mouse_entered():
	...
	$"../FamCat/Button/AnimationPlayer".current_animation == "Active":
		if PlayerGlobal.FamiliarUnlocked == true:
			$Descr.visible = true
...
func _on_Button_pressed():
	...
	$"../FamWolf/Button/AnimationPlayer".current_animation == "Active" or \
	$"../FamCat/Button/AnimationPlayer".current_animation == "Active":
		if PlayerGlobal.skill_selector == 0 and PlayerGlobal.FamItemFinder == false :
			if PlayerGlobal.FamiliarUnlocked == true:
				$Button/AnimationPlayer.play("Click")
				$Descr.visible = false
				PlayerGlobal.skill_selector = 1
				if PlayerGlobal.FamItemFinder == false:
					$Sure.visible = true
				else : pass
			else : pass
func _on_Yes_pressed():
	PlayerGlobal.skill_selector = 0
	PlayerGlobal.FamItemFinder = true
	$Sure.visible = false
func _on_No_pressed():
	PlayerGlobal.skill_selector = 0
	$Button/AnimationPlayer.play("Idle")
