...
func _on_SkillButton_mouse_entered():
	...
	if PlayerSkills.WeaponSpecialist == true and PlayerGlobal.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("ActiveHover")

func _on_SkillButton_mouse_exited():
	if !$SkillButton/Skill/AnimationPlayer.current_animation == "Click" and PlayerSkills.WeaponSpecialist == false and $Hover/YesNo/Activate.visible == false and PlayerGlobal.active_skill == false:
...
func _on_SkillButton_pressed():
	if PlayerGlobal.skillpoints > 0:
		if PlayerSkills.PowerUp == true:
			...
			if PlayerGlobal.skill_selector == 0 and PlayerSkills.WeaponSpecialist == false:
				$SkillButton/Skill/AnimationPlayer.play("Click")
				PlayerGlobal.skill_selector = 1

func _on_YesButton_pressed():
	PlayerGlobal.special_damage += 0.3
	$SkillButton/Skill/AnimationPlayer.play("Active")
